  ...
  it('scopes payout statements to the requesting recipient only', async () => {
    ...
    expect(
      deliveryPartnerItems.some(
        (item) =>
          item.productOrderId === orderId && item.entryType === CommissionEntryType.DELIVERY_FEE,
      ),
    ).toBe(true);
    expect(
      deliveryPartnerItems.some(
    ...
    expect(
      promoterItems.some(
        (item) =>
          item.productOrderId === orderId &&
          item.entryType === CommissionEntryType.PROMOTER_COMMISSION,
      ),
    ).toBe(true);
